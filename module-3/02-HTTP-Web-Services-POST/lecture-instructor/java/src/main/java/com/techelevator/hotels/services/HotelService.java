...
public class HotelService {
  ...
  public Reservation addReservation(String newReservation) {
	...
	//turns comma delimited file into a reservation object  
    Reservation reservation = makeReservation(newReservation);
    if (reservation == null) {
    	return null;
    }
    
    //create HTTP headers and set content type to application/json
    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_JSON);
    
    //declare a HTTPEntity variable
    //instantiate an HttpEntuty object and pass the reservation and header to it.
    ...
    try {
    	...
    	if (ex.getRawStatusCode() == 404) {
    		System.out.println("Wrong URL dummy!");
    	}
    	if (ex.getRawStatusCode() == 401) {
    		System.out.println("You must log in first you big dummy!");
    	}
    }
